...
세로와 가로의 길이가 각각 `M`, `N`인 방의 지도가 2차원 배열로 주어졌을 때, `1`은 장애물을 의미하고 `0`은 이동이 가능한 통로를 의미합니다. 로봇은 한 번에 임의의 `k`칸 직진과 90도 회전 중 1가지 동작을 할 수 있습니다. 로봇의 현재 위치와 방향, 목표 지점과 방향이 함께 주어집니다. 이 때, 방향은 위쪽이 `1`, 오른쪽이 `2`, 아래쪽이 `3`, 왼쪽이 `4`로 주어집니다. 로봇이 목표 지점까지 도달해 목표 방향으로 회전하는 데 필요한 동작의 수를 리턴해야 합니다.
...
- 직진은 1칸 직진이 아니라 임의의 `k`칸을 직진할 수 있습니다. 즉 한번의 직진 명령으로 장애물이 없는 한 계속 갈 수 있습니다.
